...
def main():
    ...
    if '--enable-refactored' in sys.argv:
        sys.argv.remove('--enable-refactored')
        enable_refactored_version()
        return 0
    
    if '--disable-refactored' in sys.argv:
        sys.argv.remove('--disable-refactored')
        disable_refactored_version()
        return 0
    
    if '--version-info' in sys.argv:
        sys.argv.remove('--version-info')
        show_version_info()
        return 0
    
    # Decidir qual versão executar
    if should_use_refactored_version():
        return run_refactored_main()
    else:
        return run_original_main()
