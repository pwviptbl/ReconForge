...
class VarreduraFeroxbuster:
    ...
    def __init__(self):
        ...
        self.binario_feroxbuster = obter_config('feroxbuster.binario', '/snap/bin/feroxbuster')
        ...
        self.wordlist_padrao = obter_config('feroxbuster.wordlist_padrao', 'wordlists/common.txt')
        self.wordlist_media = obter_config('feroxbuster.wordlist_media', 'wordlists/medium.txt')
    ...
    def varredura_media(self, alvo: str, wordlist: Optional[str] = None) -> Dict[str, Any]:
        """
        Executa varredura média com wordlist mais extensa
        Args:
            alvo (str): URL do alvo
            wordlist (str): Caminho para wordlist personalizada
        Returns:
        ...
        comando = [
            self.binario_feroxbuster,
            '-u', alvo,
            '-t', str(max(self.threads_padrao // 2, 25)),  # Menos threads para wordlist maior
            '--json',
            '--silent'
        ]
        ...
        if wordlist and os.path.exists(wordlist):
            comando.extend(['-w', wordlist])
        elif os.path.exists(self.wordlist_media):
            comando.extend(['-w', self.wordlist_media])
        elif os.path.exists(self.wordlist_padrao):
            comando.extend(['-w', self.wordlist_padrao])
        
        return self._executar_varredura(comando, "varredura_media")
