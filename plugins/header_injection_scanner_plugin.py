...
class HeaderInjectionScannerPlugin(VulnerabilityPlugin):
    ...
    def execute(self, target: str, context: Dict[str, Any], **kwargs) -> PluginResult:
        ...
        self.logger.info(f"🔍 Testando {len(request_nodes)} requests para Header Injection...")
                    ...
                    if 'X-Injected-Header' in response.headers:
                        found_vulns.append(
                            Vulnerability(
                                ...
                                severity="High",
                                description=f"Injecao de header detectada no parametro '{param_name}'.",
                                url=request_node.get('url'),
                                evidence=(
                                    f"Payload: {payload}\nLocation: {location}\n"
                                    f"Injected Header Found: X-Injected-Header: injected"
                                ),
                                cve="CWE-113",
